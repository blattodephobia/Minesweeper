...
{
	public partial class MineField : Panel, IMineField
	{
		...
		{
			SuspendLayout();
			ViewModel.GameState = MinesweeperGameState.GameLost;
		...
		public bool GameStarted
		{
			get
			{
				return ViewModel.GameState == MinesweeperGameState.GameInProgress;
			}
		}
		...
		public bool GameOver
		{
			get
			{
				return ViewModel.GameState != MinesweeperGameState.GameInProgress && ViewModel.GameState != MinesweeperGameState.Unknown;
			}
		}
		...
		public bool GameWon
		{
			get
			{
				return ViewModel.GameState == MinesweeperGameState.GameWon;
			}
		}
        ...
        public MineFieldViewModel ViewModel { get; set; }
        MinesweeperGameState IMineField.GameState { get => ViewModel.GameState; set => ViewModel.GameState = value; }
			...
			if (!initializing)
			{
				...
				for (int y = 0; y < Rows; y++)
				{
					for (int x = 0; x < Columns; x++)
					{
						...
						newMine.StateChanged += new System.Action<MineCell>((cell) => ViewModel.MineCellStateChanged.Execute(cell));
						mines[x, y] = newMine;
		...
		public int GetNeighbouringBombsCount(IMineSquare sender)
		{
			...
			if (!GameStarted && !GameOver)
			{
				...
				mines.DistributeMines(excludedSquares, ViewModel.RemainingBombs);
                ViewModel.GameState = MinesweeperGameState.GameInProgress;
				OnRemainingBombsCountChanged();
			}
			else
			{
